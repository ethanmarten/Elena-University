...
# --- 1. الدوال الأساسية (يجب أن تكون في الأعلى دائماً) ---
def get_local_time():
    # توقيت فلسطين (UTC+2)
    return datetime.utcnow() + timedelta(hours=2)

# --- 2. تبويبات لوحة التحكم ---
# (تأكد أن هذا الجزء يتبع الـ Tabs التي عرفتها سابقاً)

with tabs[4]:
    if st.session_state.get("user_role") == "developer":
        st.subheader("🛠️ لوحة تحكم المطور")
        # هنا تضع كود عرض المستخدمين أو السجلات
        db_dev = load_db()
        if db_dev:
...
# --- 3. السايدبار (يجب أن يكون مستقلاً وليس داخل التبويبات) ---
with st.sidebar:
    st.markdown("---")
        ...
        if expire_str:
            try:
                ...
                if total_seconds > 0:
                    ...
                    # تنفيذ الإلغاء التلقائي إذا انتهى الوقت
                    db[current_u]["status"] = "Standard"
